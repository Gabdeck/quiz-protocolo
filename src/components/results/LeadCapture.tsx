...
export function LeadCapture({ session }: { session: QuizSession }) {
  ...
  return <section className="lead-capture" aria-labelledby="lead-capture-title">
    {!canContinue ? <>
      ...
      <Link href="/oferta" className="button primary button-large" onClick={() => track("cta_clicked", { ctaLabel: "Ver minha recomendação", ctaLocation: "result_lead_capture", primaryBlocker: result.primaryBlocker })}>
